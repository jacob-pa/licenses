...
use crate::config::{GetConfig, Metadata};
...
pub fn package_licenses(
    ...
    package::dependencies(&config.common, metadata)
        .map(|package| package_to_dependency(config.search_remote, &config.keywords, package))
        .collect()
}
